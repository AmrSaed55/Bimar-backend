...
const REPO_PATH = 'C:\\Users\\Administrator\\Desktop\\Bimar\\Bimar-backend';  // Your project directory path
const PORT = 3000;

app.post('/webhook', (req, res) => {
    // Verify signature if using GitHub
    const signature = req.headers['x-hub-signature'];
    const hmac = crypto.createHmac('sha1', SECRET);
    const digest = 'sha1=' + hmac.update(JSON.stringify(req.body)).digest('hex');
    
    if (signature === digest) {
        exec('cd /d ' + REPO_PATH + ' && git pull && pm2 reload all', (error, stdout, stderr) => {
            if (error) {

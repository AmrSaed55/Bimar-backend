...
const submitRating = async(req,res)=>{
    try{
        ...
        if(!hasCompletedAppointment) throw "you must have at least one completed appointment with this doctor to rate him";
        ...
        const newAverageRating = (doctor.ratings.averageRating * doctor.ratings.totalRatings + rating) / totalRatings;
...
const getDoctorRatings = async (req, res) => {
    try {
        ...
        // Get doctor details
        const doctor = await Doctor.findById(doctorId)
            .select('ratings doctorName field');
...
const deleteRating = async (req, res) => {
    try {
        ...
        if (totalRatings > 0) {
            const currentTotal = doctor.ratings.averageRating * doctor.ratings.totalRatings;
